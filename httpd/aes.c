#include <stdio.h>
#include <stdlib.h>
#include <sys/stat.h>

#include "aes.h"

#define BUFSIZE (16*512)

#ifdef SELF_AES

#define FFSWAP(type,a,b) do{type SWAP_tmp= b; b= a; a= SWAP_tmp;}while(0)
static const uint8_t rcon[10] = {
	0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

static uint8_t     sbox[256];
static uint8_t inv_sbox[256];
#ifdef CONFIG_SMALL
static uint32_t enc_multbl[1][256];
static uint32_t dec_multbl[1][256];
#else
static uint32_t enc_multbl[4][256];
static uint32_t dec_multbl[4][256];
#endif

static inline void addkey(uint64_t dst[2], uint64_t src[2], uint64_t round_key[2])
{
	dst[0] = src[0] ^ round_key[0];
	dst[1] = src[1] ^ round_key[1];
}

static void subshift(uint8_t s0[2][16], int s, uint8_t *box)
{
	uint8_t (*s1)[16]= s0[0] - s;
	uint8_t (*s3)[16]= s0[0] + s;
	s0[0][0]=box[s0[1][ 0]]; s0[0][ 4]=box[s0[1][ 4]]; s0[0][ 8]=box[s0[1][ 8]]; s0[0][12]=box[s0[1][12]];
	s1[0][3]=box[s1[1][ 7]]; s1[0][ 7]=box[s1[1][11]]; s1[0][11]=box[s1[1][15]]; s1[0][15]=box[s1[1][ 3]];
	s0[0][2]=box[s0[1][10]]; s0[0][10]=box[s0[1][ 2]]; s0[0][ 6]=box[s0[1][14]]; s0[0][14]=box[s0[1][ 6]];
	s3[0][1]=box[s3[1][13]]; s3[0][13]=box[s3[1][ 9]]; s3[0][ 9]=box[s3[1][ 5]]; s3[0][ 5]=box[s3[1][ 1]];
}

static inline int mix_core(uint32_t multbl[4][256], int a, int b, int c, int d)
{
#ifdef CONFIG_SMALL
#define ROT(x,s) ((x<<s)|(x>>(32-s)))
	return multbl[0][a] ^ ROT(multbl[0][b], 8) ^ ROT(multbl[0][c], 16) ^ ROT(multbl[0][d], 24);
#else
	return multbl[0][a] ^ multbl[1][b] ^ multbl[2][c] ^ multbl[3][d];
#endif
}

static inline void mix(uint8_t state[2][4][4], uint32_t multbl[4][256], int s1, int s3)
{
	((uint32_t *)(state))[0] = mix_core(multbl, state[1][0][0], state[1][s1  ][1], state[1][2][2], state[1][s3  ][3]);
	((uint32_t *)(state))[1] = mix_core(multbl, state[1][1][0], state[1][s3-1][1], state[1][3][2], state[1][s1-1][3]);
	((uint32_t *)(state))[2] = mix_core(multbl, state[1][2][0], state[1][s3  ][1], state[1][0][2], state[1][s1  ][3]);
	((uint32_t *)(state))[3] = mix_core(multbl, state[1][3][0], state[1][s1-1][1], state[1][1][2], state[1][s3-1][3]);
}

static inline void crypt(AVAES *a, int s, uint8_t *sbox, uint32_t *multbl)
{
	int r;
	
	for(r=a->rounds-1; r>0; r--){
		mix(a->state, multbl, 3-s, 1+s);
		addkey(a->state[1], a->state[0], a->round_key[r]);
	}
	subshift(a->state[0][0], s, sbox);
}

void av_aes_crypt(AVAES *a, uint8_t *dst, uint8_t *src, int count, uint8_t *iv, int decrypt)
{
	while(count--){
		addkey(a->state[1], src, a->round_key[a->rounds]);
		if(decrypt) {
			crypt(a, 0, inv_sbox, dec_multbl);
			if(iv){
				addkey(a->state[0], a->state[0], iv);
				memcpy(iv, src, 16);
			}
			addkey(dst, a->state[0], a->round_key[0]);
		}else{
			if(iv) addkey(a->state[1], a->state[1], iv);
			crypt(a, 2,     sbox, enc_multbl);
			addkey(dst, a->state[0], a->round_key[0]);
			if(iv) memcpy(iv, dst, 16);
		}
		src+=16;
		dst+=16;
	}
}

static void init_multbl2(uint8_t tbl[1024], int c[4], uint8_t *log8, uint8_t *alog8, uint8_t *sbox)
{
	int i, j;
	for(i=0; i<1024; i++){
		int x= sbox[i>>2];
		if(x) tbl[i]= alog8[ log8[x] + log8[c[i&3]] ];
	}
#ifndef CONFIG_SMALL
	for(j=256; j<1024; j++)
		for(i=0; i<4; i++)
			tbl[4*j+i]= tbl[4*j + ((i-1)&3) - 1024];
#endif
}

// this is based on the reference AES code by Paulo Barreto and Vincent Rijmen
int av_aes_init(AVAES *a, const uint8_t *key, int key_bits, int decrypt) 
{
	int i, j, t, rconpointer = 0;
	uint8_t tk[8][4];
	int KC= key_bits>>5;
	int rounds= KC + 6;
	uint8_t  log8[256];
	uint8_t alog8[512];

	if(!enc_multbl[0][sizeof(enc_multbl)/sizeof(enc_multbl[0][0])-1]){
		j=1;
		for(i=0; i<255; i++){
			alog8[i]=
			alog8[i+255]= j;
			log8[j]= i;
			j^= j+j;
			if(j>255) j^= 0x11B;
		}
		for(i=0; i<256; i++){
			j= i ? alog8[255-log8[i]] : 0;
			j ^= (j<<1) ^ (j<<2) ^ (j<<3) ^ (j<<4);
			j = (j ^ (j>>8) ^ 99) & 255;
			inv_sbox[j]= i;
			sbox    [i]= j;
		}
		init_multbl2(dec_multbl[0], (int[4]){0xe, 0x9, 0xd, 0xb}, log8, alog8, inv_sbox);
		init_multbl2(enc_multbl[0], (int[4]){0x2, 0x1, 0x1, 0x3}, log8, alog8, sbox);
	}
	
	if(key_bits!=128 && key_bits!=192 && key_bits!=256)
		 return -1;
	
	a->rounds= rounds;
	
	memcpy(tk, key, KC*4);
	
	for(t= 0; t < (rounds+1)*16;) {
		memcpy(a->round_key[0][0]+t, tk, KC*4);
		t+= KC*4;
		
		for(i = 0; i < 4; i++)
		tk[0][i] ^= sbox[tk[KC-1][(i+1)&3]];
		tk[0][0] ^= rcon[rconpointer++];
		
		for(j = 1; j < KC; j++){
			if(KC != 8 || j != KC>>1)
				for(i = 0; i < 4; i++) tk[j][i] ^= tk[j-1][i];
			else
				for(i = 0; i < 4; i++) tk[j][i] ^= sbox[tk[j-1][i]];
		}
	}
	
	if(decrypt){
		for(i=1; i<rounds; i++){
			uint8_t tmp[3][16];
			memcpy(tmp[2], a->round_key[i][0], 16);
			subshift(tmp[1], 0, sbox);
			mix(tmp, dec_multbl, 1, 3);
			memcpy(a->round_key[i][0], tmp[0], 16);
		}
	}else{
		for(i=0; i<(rounds+1)>>1; i++){
			for(j=0; j<16; j++)
				 FFSWAP(int, a->round_key[i][0][j], a->round_key[rounds-i][0][j]);
		}
	}
	
	return 0;
}

#else
int av_aes_init(AES_KEY *a, const uint8_t *key, int key_bits, int decrypt) 
{
	memset(a, 0, av_aes_size);
	uint8_t userkey[17] = {0,};
	memcpy(userkey, key, strlen(key));
	int i;
	printf("av_aes_init: ");
	for (i=0; i<16;i++)
		printf("%x ", userkey[i]);
	printf("\n");

	if (decrypt)
		return AES_set_decrypt_key(userkey, key_bits, a);
	else
		return AES_set_encrypt_key(userkey, key_bits, a);
}

void av_aes_crypt(AVAES *a, uint8_t *dst, uint8_t *src, int count, uint8_t *iv, int decrypt)
{
	if (decrypt){
		if (iv == NULL) {
			int i;
			for (i=0; i<count; i++){
				int p = i * AES_BLOCK_SIZE;
				AES_ecb_encrypt(src + p, dst + p, a, AES_DECRYPT);
			}
		}
		else
			AES_cbc_encrypt(src, dst, count * AES_BLOCK_SIZE, a, iv, AES_DECRYPT);

	}
	else {
		if (iv == NULL) {
			int i;
			for (i=0; i<count; i++) {
				int p = i * AES_BLOCK_SIZE;
				AES_ecb_encrypt(src + p, dst + p, a, AES_ENCRYPT);
			}
		}
		else
			AES_cbc_encrypt(src, dst, count * AES_BLOCK_SIZE, a, iv, AES_ENCRYPT);
	}
}
#endif

const int av_aes_size= sizeof(AVAES);
typedef struct AesFile{
	AesHead Head;
	FILE *fp;
	uint8_t ivec[16];
	uint8_t tmpbuffer[BUFSIZE];
	int buffer_size;
	size_t position;
	AVAES *aesc;
	int decrypt;
} AesFile;

////////////////////////////////////////////////////////////////////////////////
struct AesFile* AesOpenFile(const char *aesfile, uint8_t *password, int decrypt)
{
	FILE *fh_in;
	AesFile *aes = NULL;
	if (decrypt) {
		if ((fh_in = fopen(aesfile, "rb")) == NULL)
			return NULL;
		
		aes = (AesFile*)malloc(sizeof(AesFile));
		memset(aes, 0, sizeof(AesFile));
		aes->fp = fh_in;

		if (ReadAesHead(fh_in, &aes->Head, password) == -1){
			printf("%s = %s error\n", aesfile, password);
			fclose(fh_in);
			free(aes);
			return NULL;
		}
		aes->aesc = malloc(av_aes_size);
		memset(aes->aesc, 0, av_aes_size);
		av_aes_init(aes->aesc, password, 128, 1);
	}
	else {
		if ((fh_in = fopen(aesfile, "wb")) == NULL)
			return NULL;
		
		aes = (AesFile*)malloc(sizeof(AesFile));
		memset(aes, 0, sizeof(AesFile));
		aes->fp = fh_in;

		memset(&aes->Head, 0, sizeof(AesHead));
		aes->Head.Version = 1;
	
		fseek(aes->fp, 0, SEEK_END);
		aes->Head.TLen = ftell(aes->fp);
		fseek(aes->fp, 0, SEEK_SET);

		aes->aesc = malloc(av_aes_size);
		memset(aes->aesc, 0, av_aes_size);
		av_aes_init(aes->aesc, password, 128, 0);
		memcpy(aes->Head.AesKey, password, strlen(password));
		av_aes_crypt(aes->aesc, aes->Head.AesKey, aes->Head.AesKey, 1, NULL, 0);
		fwrite(&aes->Head, sizeof(AesHead), 1, aes->fp);
		memset(aes->aesc, 0, av_aes_size);
		av_aes_init(aes->aesc, password, 128, 0);
	}
	memset(aes->ivec, 0, 16);
	strcpy(aes->ivec, "294966");
	aes->decrypt = decrypt;

	return aes;
}

int AesWriteFile(struct AesFile *aes, void *ptr, size_t size)
{
	if (aes->decrypt) return 0;
	av_aes_crypt(aes->aesc, ptr, ptr, size, aes->ivec, 0);
	aes->position += fwrite(ptr, 1, size * 16, aes->fp); 	
	
	return size * 16;
}

int AesReadFile(struct AesFile *aes, void *ptr, size_t size)
{
	int pos = 0;
	int data_len, need_size;
	uint8_t databuf[BUFSIZE];
	if (aes->decrypt == 0) return 0;
	
	if (aes->buffer_size > size) {
		memcpy(ptr, aes->tmpbuffer, size);
		pos = size;
		aes->buffer_size -= size;
		memcpy(aes->tmpbuffer, aes->tmpbuffer + size, aes->buffer_size); 
	}
	else if (aes->buffer_size != 0){
		memcpy(ptr, aes->tmpbuffer, aes->buffer_size);
		pos = aes->buffer_size;
		aes->buffer_size = 0;		
	}
	while (pos < size && !feof(aes->fp)) {
		memset(databuf, 0, BUFSIZE);
		fread(databuf, BUFSIZE, 1, aes->fp);
		av_aes_crypt(aes->aesc, databuf, databuf, 512, aes->ivec, 1);
		data_len = aes->Head.TLen - aes->position > BUFSIZE ? BUFSIZE : aes->Head.TLen - aes->position;
		aes->position += data_len;
		if (size - pos > data_len)
			need_size = data_len;
		else {
			need_size = size - pos;
			aes->buffer_size = data_len - need_size;
			memcpy(aes->tmpbuffer, databuf + need_size, aes->buffer_size);
		}
		memcpy(ptr + pos, databuf, need_size);
		pos += need_size;
	}
	
	return pos;
}

void AesCloseFile(struct AesFile *aes)
{
	if (aes->decrypt == 0){
		fseek(aes->fp, 0, SEEK_SET);
		aes->Head.TLen = aes->position;
		fwrite(&aes->Head, sizeof(AesHead), 1, aes->fp);
	}
	fclose(aes->fp);
	free(aes->aesc);
	free(aes);
}

int AesEncryptFile(const char *infile, const char *outfile, uint8_t *KeyStr)
{
	FILE *fh_in, *fh_out;
	long len;
	uint8_t databuf[BUFSIZE];
	uint8_t ivec[16];
	AVAES *aesc = malloc(av_aes_size);

	if ((fh_in = fopen(infile, "rb")) == NULL)
		return -1;
	if ((fh_out = fopen(outfile, "wb")) == NULL)
	   return -1;
	   
	AesHead aeshead;
	memset(&aeshead, 0, sizeof(AesHead));
	aeshead.Version = 1;
	
	fseek(fh_in, 0, SEEK_END);
	aeshead.TLen = ftell(fh_in);
	fseek(fh_in, 0, SEEK_SET);

	memset(aesc, 0, av_aes_size);
	av_aes_init(aesc, KeyStr, 128, 0);
	memcpy(aeshead.AesKey, KeyStr, strlen(KeyStr));
	av_aes_crypt(aesc, aeshead.AesKey, aeshead.AesKey, 1, NULL, 0);
	int i;
	printf("AesEncryptFile: ");
	for (i=0; i<16; i++)
		printf("%x ", aeshead.AesKey[i]);
	printf("\n");
	fwrite(&aeshead, sizeof(aeshead), 1, fh_out);

	printf("AesEncrptyFile (%s, %s, %s, len=%ld, sizeof(aeshead)=%d\n", infile, outfile, KeyStr, aeshead.TLen, sizeof(aeshead));
	memset(ivec, 0, 16);
	strcpy(ivec, "294966");
	memset(aesc, 0, av_aes_size);
	av_aes_init(aesc, KeyStr, 128, 0);
	while( !feof(fh_in) ) {
		memset(databuf, 0, BUFSIZE);
		fread(databuf, BUFSIZE, 1, fh_in);
//		av_aes_crypt(aesc, databuf, databuf, 512, NULL, 0);
		av_aes_crypt(aesc, databuf, databuf, 512, ivec, 0);
		fwrite(databuf, BUFSIZE, 1, fh_out);
	}
	fclose(fh_in);
	fclose(fh_out);
	free(aesc);
	
	return 0;
}
 
int ReadAesHead(FILE *fp, AesHead *aeshead, uint8_t *KeyStr)
{
	if (fp==NULL)
		return -1;
	if (fread(aeshead, 1, sizeof(AesHead), fp) != sizeof(AesHead))
		return -1;

	if (aeshead->Version != 1)
		return -1;

	uint8_t aesKey[17];
	AVAES *aesc = malloc(av_aes_size);

	memset(aesKey, 0, 17);
	memset(aesc, 0, av_aes_size);
	av_aes_init(aesc, KeyStr, 128, 1);	
	av_aes_crypt(aesc, aesKey, aeshead->AesKey, 1, NULL, 1);
	int i;
	printf("%s: ", __FUNCTION__);
	for (i=0; i<16; i++)
		printf("%x ", aeshead->AesKey[i]);
	printf("\n\n");
	for (i=0; i<16; i++)
		printf("%x ", aesKey[i]);
	printf("\n");
	printf("aesKey=%s, keyStr=%s, aeshead.TLen=%ld\n", aesKey, KeyStr, aeshead->TLen);
	free(aesc);
	if (memcmp(aesKey, KeyStr, strlen(KeyStr)) == 0) {
		return 0;
	}
	
	return -1;
}

int AesDecryptFile(const char *infile, const char *outfile, uint8_t *KeyStr)
{
	FILE *fh_in, *fh_out;
	if ((fh_in = fopen(infile, "rb")) == NULL)
		return -1;
	long len;
	AesHead aeshead;
	uint8_t databuf[BUFSIZE];

	if (ReadAesHead(fh_in, &aeshead, KeyStr) == -1){
		fclose(fh_in);
		return -1;
	}
	printf("open %s\n", outfile);
	if ((fh_out = fopen(outfile, "wb")) == NULL)
		return -1;
	
	uint8_t ivec[16];
	AVAES *aesc = malloc(av_aes_size);

	memset(ivec, 0, 16);
	strcpy(ivec, "294966");
	memset(aesc, 0, av_aes_size);
	av_aes_init(aesc, KeyStr, 128, 1);
		
	while( !feof(fh_in) ) {
		memset(databuf, 0, BUFSIZE);
		fread(databuf, BUFSIZE, 1, fh_in);
//		av_aes_crypt(aesc, databuf, databuf, 512, NULL, 1);
		av_aes_crypt(aesc, databuf, databuf, 512, ivec, 1);
		aeshead.TLen -= fwrite(databuf, 1, aeshead.TLen > BUFSIZE ? BUFSIZE: aeshead.TLen, fh_out);
	}
	free(aesc);
	fclose(fh_in);
	fclose(fh_out);
	return 0;
}

#ifdef TEST
int main(int argc, char **argv)
{
	if (argv[1][0] == 'e')
		AesEncryptFile(argv[2], argv[3], argv[4]);
	else
		AesDecryptFile(argv[2], argv[3], argv[4]);
/*
 struct AesFile *aes = AesOpenFile(argv[2], argv[3], 1);
	char tempstring[1025];
	while (1) {
		int len = AesReadFile(aes, tempstring, 1024);
		if (len <=0) 
			break;
		tempstring[len] = 0;
		printf("tempstring=%s\n", tempstring);
	}
*/

	return 0;
}
#endif
