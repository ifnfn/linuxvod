...
int rtp_set_remote_url(URLContext *h, const char *uri)
{
	...
	url_split(NULL, 0, NULL, 0, hostname, sizeof(hostname), &port, path, sizeof(path), uri);
...
static int rtp_open(URLContext *h, const char *uri, int flags)
{
	...
	build_udp_url(buf, sizeof(buf),hostname, port, local_port, is_multicast, ttl);
	...
	build_udp_url(buf, sizeof(buf), hostname, port + 1, local_port + 1, is_multicast, ttl);
...
static int rtp_read(URLContext *h, uint8_t *buf, int size)
{
	...
	for(;;)
	{
		...
		len = recvfrom (s->rtp_fd, buf, size, 0, (struct sockaddr *)&from, &from_len);
		if (len < 0)
		{
	...
	for(;;)
	{
		...
		if (n > 0)
		{
			...
			{
				from_len = sizeof(from);
				len = recvfrom (s->rtcp_fd, buf, size, 0, (struct sockaddr *)&from, (socklen_t*)&from_len);
				int x;
				for (x=0;x<len;x++)
					printf("%X ", buf[x]);
				printf("\n");
			...
			{
				from_len = sizeof(from);
				len = recvfrom (s->rtp_fd, buf, size, 0, (struct sockaddr *)&from, (socklen_t*)&from_len);
				if (len < 0)
				{
					if (errno == EAGAIN || errno == EINTR)
						continue;
