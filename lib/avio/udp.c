...
static int udp_open(URLContext *h, const char *uri, int flags)
{
	...
	len = sizeof(my_addr1);
	getsockname(udp_fd, (struct sockaddr *)&my_addr1, (socklen_t*)&len);
	...
	if (s->is_multicast)
	{
		...
		{
			/* output */
			if (setsockopt(udp_fd, IPPROTO_IP, IP_MULTICAST_TTL, (SETPOINT)&s->ttl, sizeof(s->ttl)) < 0) {
			...
			if (setsockopt(udp_fd, IPPROTO_IP, IP_ADD_MEMBERSHIP, (SETPOINT)&s->mreq, sizeof(s->mreq)) < 0) {
	...
	if (is_output)
	{
		...
		tmp = UDP_TX_BUF_SIZE;
		if (setsockopt(udp_fd, SOL_SOCKET, SO_SNDBUF, (SETPOINT)&tmp, sizeof(tmp)) < 0) {
...
static int udp_read(URLContext *h, uint8_t *buf, int size)
{
	...
	for(;;)
	{
		...
		len = recvfrom (s->udp_fd, buf, size, 0, (struct sockaddr *)&from, (socklen_t*)&from_len);
		if (len < 0) {
...
static int udp_write(URLContext *h, uint8_t *buf, int size)
{
	...
	for(;;)
	{
		ret = sendto (s->udp_fd, buf, size, 0, (struct sockaddr *) &s->dest_addr,
#ifndef CONFIG_IPV6
					sizeof (s->dest_addr));
#else
					s->dest_addr_len);
...
static int udp_close(URLContext *h)
{
	...
	if (s->is_multicast && !(h->flags & URL_WRONLY))
	{
		if (setsockopt(s->udp_fd, IPPROTO_IP, IP_DROP_MEMBERSHIP, (SETPOINT)&s->mreq, sizeof(s->mreq)) < 0)
